pipeline {
    ...
    stages {
        stage('Checkout Source') {
            steps {
                git 'https://github.com/DeanKelly751/jenkins-kubernetes-pipeline.git'
            }
        }
            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: '316e0e5e-cd67-4266-8a21-9a6ef36a1bd0', passwordVariable: 'pass', usernameVariable: 'user')]) {
                        sh "git push https://$user:$pass@github.com/DeanKelly751/jenkins-kubernetes-pipeline.git main"
                    }
                }
            }
        }
